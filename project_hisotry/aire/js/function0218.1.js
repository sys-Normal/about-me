...
$(function(){
    ...
    var $wrap = $("#wrap");
    var pageTop = function(){
        value[0] = $wrap.find("header").offset().top; - 69
        value[1] = $wrap.find("#about").offset().top; - 69
        value[2] = $wrap.find("#service").offset().top; -69
        // value[3] = $wrap.find("#number").offset().top;
        value[3] = $wrap.find("#news").offset().top; - 69
        value[4] = $wrap.find("footer").offset().top;
    };
    ...
    $(window).on("scroll",function(){
        ...
        if(scrollTop>=1){
            $Menu.addClass("fixed");
            $Menu.css("marginTop",-70);
        }else{
            $Menu.css("marginTop",0);
        }

        if(scrollTop>$main_home.height()/2){
...
$(function(){
        ...
        intervalKey = setInterval(function(){
            if($(window).width()<768){
                $slideBar.stop().animate({
                    left : -$slideBar.width()/7 * 2
                },1000, function(){
                    $slideBar.children("li").eq(0).appendTo($slideBar); 
                    $slideBar.css({left:-$slideBar.width()/7});
                });
    ...
    console.log($slideBar.width());
    console.log($slideBar.children("li").width());
